...
const validRect = {
	...
	stroke: "#000", strokeWidth: 2, strokeDashType: "solid",
	fill: "#fff",
	text: "hello", textType: "text", textAlign: "center",
	verticalAlign: "middle", fontColor: "#000", fontSize: 16,
...
describe("validateRectDoc", () => {
	...
	it("textAlign が不正な値はエラー", () => {
		const errors = validateRectDoc({ ...validRect, textAlign: "justify" }, "root");
		expect(errors.some(e => e.path === "root.textAlign")).toBe(true);
	});

	it("verticalAlign が不正な値はエラー", () => {
		const errors = validateRectDoc({ ...validRect, verticalAlign: "baseline" }, "root");
		expect(errors.some(e => e.path === "root.verticalAlign")).toBe(true);
	});

	it("strokeDashType が不正な値はエラー", () => {
		const errors = validateRectDoc({ ...validRect, strokeDashType: "double" }, "root");
