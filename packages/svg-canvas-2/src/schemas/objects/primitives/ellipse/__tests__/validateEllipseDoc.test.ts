...
const validEllipse = {
	...
	fill: "transparent",
	text: "", textType: "text", textAlign: "center", verticalAlign: "middle",
	fontColor: "#000", fontSize: 14, fontFamily: "sans-serif", fontWeight: "normal",
};
...
describe("validateEllipseDoc", () => {
	...
	it("ry が数値でない場合はエラー", () => {
		const errors = validateEllipseDoc({ ...validEllipse, ry: undefined }, "root");
		expect(errors.some(e => e.path === "root.ry")).toBe(true);
	});

	it("textType が不正な値はエラー", () => {
		const errors = validateEllipseDoc({ ...validEllipse, textType: "html" }, "root");
