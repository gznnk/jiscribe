...
const validSticky = {
	...
	fill: "#fef9c3",
	text: "memo", textType: "text", textAlign: "center",
	verticalAlign: "middle", fontColor: "#000", fontSize: 14,
	fontFamily: "Noto Sans JP", fontWeight: "normal",
};
...
describe("validateStickyDoc", () => {
	...
	it("必須の width が数値でない場合はエラー", () => {
		const errors = validateStickyDoc({ ...validSticky, width: undefined }, "root");
		expect(errors.some(e => e.path === "root.width")).toBe(true);
	});

	it("必須の height が数値でない場合はエラー", () => {
		const errors = validateStickyDoc({ ...validSticky, height: "120px" }, "root");
		expect(errors.some(e => e.path === "root.height")).toBe(true);
	});

	it("textAlign が不正な値はエラー", () => {
		const errors = validateStickyDoc({ ...validSticky, textAlign: "justify" }, "root");
		expect(errors.some(e => e.path === "root.textAlign")).toBe(true);
	});

	it("verticalAlign が不正な値はエラー", () => {
		const errors = validateStickyDoc({ ...validSticky, verticalAlign: "baseline" }, "root");
		expect(errors.some(e => e.path === "root.verticalAlign")).toBe(true);
	});

	it("fill が string でない場合はエラー", () => {
		const errors = validateStickyDoc({ ...validSticky, fill: 0xfef9c3 }, "root");
