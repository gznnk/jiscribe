...
| `text-editor`                   | the text editing overlay                 | shared by shapes and connector labels. The surface inside it is a contenteditable DIV for a shape and a TEXTAREA for a connector label; `CanvasDriver.textEditorSurface()` matches either |
...
Shape library buttons use part: `item:<presetId>` (for example `item:rect`). The
shape library sidebar (`stencil-library-panel`, in the DOM only while open) uses
part: `section:<categoryId>` for a section's disclosure header and part: `close`
for the button that shuts it.
...
`selectors.toolButton` and `selectors.shapeItem` are scoped to the bar for that
reason: an open sidebar holds a twin of every pinned preset, carrying the same
`title` and the same `item:<presetId>`, so an unscoped selector matches two
elements. The sidebar's copy is `selectors.stencilLibraryPanelItem`.
...
await page.click('[data-id="toolbar"] button[title="Rectangle"]');
await drag(...);
