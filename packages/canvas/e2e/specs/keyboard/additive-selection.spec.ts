...
/**
 * Additive selection via Ctrl/Meta/Shift+click.
 *
 ...
 * clicks go through canvas.ctrlClickAt / canvas.shiftClickAt, which apply the
 * coordinate transform.
 */

test.describe("multi-selection (Ctrl+click / Shift+click / Ctrl+A)", () => {
	...
	test("adds to the selection on Shift+click and moves them together", async ({
		canvas,
	}) => {
		// A: center (370,260), B: center (630,260)
		const a = await canvas.drawShape(
		...
		await canvas.selectAt({ x: 370, y: 260 });
		await canvas.shiftClickAt({ x: 630, y: 260 });
