...
export const DiamondShapeFactory: ShapeFactory = {
	createDoc(position, overrides) {
		const width = numberOverride(overrides?.width, DIAMOND_DOC_DEFAULTS.width);
		const height = numberOverride(
			overrides?.height,
			DIAMOND_DOC_DEFAULTS.height,
		);
		return {
	...
	calcDimensions(overrides) {
		return {
			halfWidth:
				numberOverride(overrides?.width, DIAMOND_DOC_DEFAULTS.width) / 2,
			halfHeight:
				numberOverride(overrides?.height, DIAMOND_DOC_DEFAULTS.height) / 2,
		};
	},
