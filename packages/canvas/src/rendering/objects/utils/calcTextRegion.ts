...
} from "../registry/ObjectTextRegionRegistry";
 ...
 * @param context - The drawing context a calculator sizing a region from its own text measures with; required even where the calculator is not, so a type registering one later cannot be handed nothing
...
export const calcTextRegion = (
	...
	context: ObjectTextRegionContext,
): Rect => calculator?.(state, slotId, context) ?? calcFullTextRegion(state);
 ...
 * no calculator — going through {@link calcTextRegion} would make them produce a
 * drawing context they have no use for.
 *
