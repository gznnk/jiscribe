...
型の意図を反映しなくなる。パーサーが該当 ID を弾くので、それを含むドキュメントは
読み込まれない。
...
正である。導出でも宣言でも、この後に `derivePropertyPanel` が、定義がそれを含意
する型に対して自動高さの行を layout セクションへ、縦基準の行を text セクションへ
足すので、宣言側がこの 2 行を書く必要はない。宣言は組み込みの行種を並べ、どれも
