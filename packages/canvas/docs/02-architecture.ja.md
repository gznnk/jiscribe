...
1. **レイヤー分離**: 下から順にデータ層（schemas / states）・描画層（rendering）・制御層（controllers）を明確に分離する。**制御層が最上位**で、描画層のコンポーネントを組み立てて画面を作る
...
依存: `rendering → states / schemas`（Props の型に加え、`EndpointRef` などの schema 型や `AUTO_COLOR` などの定数も参照する）。
...
依存: `controllers → rendering → states / schemas`。制御層は描画層の**上**に乗るので、UI コントローラが描画層のコンポーネント（例: `PendingConnectorOverlay` → `ConnectorRenderer`、`ArrowHeadIconPreview` → `Arrow`）や registry Context（`RenderingRegistriesProvider` など）を import するのは、上位が下位の部品を組み立てる通常の合成であって例外ではない。
