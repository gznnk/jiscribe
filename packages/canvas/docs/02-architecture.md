...
- **Pure reducer/handler/util tree** (cannot read React context) → carried on `CanvasControllerState.registries`; every pure function reads `state.registries.*`. Leaf utils without `state` receive the specific sub-registry as an argument.
...
> **On `CanvasMapper`**: whole-document `CanvasDoc ↔ CanvasState` conversion must invoke each shape's Mapper polymorphically, so `states/canvas/CanvasMapper.ts` takes an `ObjectMapperRegistry` argument (`canvasToState(doc, mapper)` / `canvasToDoc(state, mapper)`) rather than reaching for a global — the caller passes the canvas's own `state.registries.objectMapper`. `ObjectMapperRegistry` is the only registry the `states/` layer depends on (colocated with the mappers it serves), so this is not a cross-layer exception.
