...
- タッチは**指がどこに落ちたか**で分担する。`CanvasRoot` の `touch-action` は `none` から
  `pan-x pan-y` に緩み、背景の 1 本指ドラッグはホストページをスクロールする（ハンドラ側も
  この形のドラッグでビューポートをパンしない — さもないとページに動く余地がないドラッグが
  キャンバスを動かしてしまう）。図形の上で始まったタッチは図形ドラッグのまま通る。実効的な
  主張は `useCooperativeTouchClaim` の非 passive な `touchstart` ガードで、Chromium と WebKit
  が SVG 内部要素の `touch-action` を無視するため。ビュー自体の移動は 2 本指で、これは
  どちらの `touch-action` 値でもブラウザに渡らないので、ピンチはキャンバスのパン・ズームのまま
