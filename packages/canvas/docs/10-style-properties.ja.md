...
| `TextSlotStyleProperty`     | テキストの書式（fontSize / textAlign など）。そのフィールドを受け付ける text type のオブジェクト（`textStyleKeysOf`）に適用し、スロット単位で書く（スロットが選ばれていればそれだけ、無ければ全スロット。編集中に文字範囲が選ばれていればその範囲だけ。`"source"` の本文は文字単位の書式を持たないので、範囲を選んでいてもスロット全体に当たる） |
