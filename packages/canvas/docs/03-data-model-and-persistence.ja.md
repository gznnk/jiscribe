...
| 段                                                                                  | 見るもの                                                                                                                       | 結果                                                           |
| ----------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------- |
| `stripUnknownContent`                                                               | 列挙型フィールドの未知の値・未登録の型のオブジェクト                                                                           | 値は削除、オブジェクトは不透明として保持。どちらも warning     |
| `checkStructure`                                                                    | 文書の骨格: `version`・旧トップレベルの `connectors`・`background`・`view`・`root` 各要素の `id` / `type`・group の `children` | error                                                          |
| `registry.validate`（型の `validateDoc` と、`features` + `extraKeys` が認める名前） | 型が持つフィールドの値・型が持たないフィールド名                                                                               | 値の不正は error、未知の名前は `unknownKeyPath` を持つ warning |
...
2. **構造検証 `checkStructure`** — 各ノードの型・必須フィールドを検証。型別の検証は
...
3. **意味検証 `checkSemantics`** — 文書全体を横断しないと判断できない整合性を検証。
