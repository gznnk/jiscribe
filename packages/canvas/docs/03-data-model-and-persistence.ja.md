...
| 段                                                                                    | 見るもの                                                                                                                                                                                            | 結果                                                                       |
| ------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- |
| `stripUnknownContent`                                                                 | 列挙型フィールドに書かれた既知の集合の外の文字列・未登録の型のオブジェクト                                                                                                                          | 値は削除、オブジェクトは不透明として保持。どちらも warning                 |
...
1. **未知の内容の除去 `stripUnknownContent`** — 列挙型フィールドの未知の値を取り除く。対象は既知の集合の外の
   文字列（新しいビルド向けに書かれた文書が持つもの）だけで、別の型の値（`textAlign: 1`）は破損なので
   バリデータに任せて error にする。除去はエラーにせず
