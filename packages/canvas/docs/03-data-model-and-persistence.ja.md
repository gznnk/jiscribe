...
### `meta.comments` は会話であって絵ではない

`meta.comments` が持つのは、そのオブジェクトに残されたコメントスレッド。1 つの
スレッドは id と解決状態（`resolved` / `resolvedBy`。開いている間はどちらも無い）、
そして 1 件以上のコメント（`author` / `body` / `createdAt`。編集されれば `editedAt`）
を持つ。`meta` は自由形式で読み込み時に検証されないので、読む側は
`readCommentThreads`（`@jiscribe/doc`）で絞り込む。整っていないスレッドはそこで
落ち、そのオブジェクトへの次の書き戻しで文書からも消える。スレッドからは何も
描かれないので、オブジェクトを複製してもスレッドは付いてこない（`cloneObjects`）
——スレッドは残された当のオブジェクトのもので、複製は自分の会話を始める。文書の
一部なので、`.jis` を埋め込む書き出し（`.jis.svg` / `.jis.png`）には一緒に入る。
...
オブジェクトの `meta` は `META_PROPERTY_UPDATE`、そのコメントスレッドは第 5 の経路として `COMMENT_UPDATE`。
どれも commit 機微は二重に持たず、コミット末尾
...
`COMMENT_UPDATE` は運ぶものと commit の仕方が違う。1 つの文字列フィールドではなくスレッド配列への op で、
対象は選択から読まずに op が名指しする（パネルはマーカーから開き、そのとき ObjectMenu は出ていない）。
プレビュー段も無い——投稿されたコメントは投稿済みなので、スレッドが変わる op はその場で必ずコミットし、
まとめて 1 件にする coalesce もしない。
