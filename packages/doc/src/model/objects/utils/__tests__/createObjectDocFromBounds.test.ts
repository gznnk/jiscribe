...
describe("createObjectDocFromBounds", () => {
	describe("polyline", () => {
		it("distance below minSize → null", () => {
			...
			expect(
				createObjectDocFromBounds("polyline", 0, 0, 2, 2, objectFactory),
			).toBeNull();
		});
	...
	describe("rect", () => {
		it("width below minSize → null", () => {
			expect(
				createObjectDocFromBounds("rect", 0, 0, 3, 100, objectFactory),
			).toBeNull();
		});

		it("height below minSize → null", () => {
			expect(
				createObjectDocFromBounds("rect", 0, 0, 100, 3, objectFactory),
			).toBeNull();
		});
		...
		it("a custom minSize can be specified", () => {
			...
			expect(
				createObjectDocFromBounds("rect", 0, 0, 15, 100, objectFactory, {}, 20),
			).toBeNull();
		});
	});
