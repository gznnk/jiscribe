...
import { createBuiltinObjectFactoryRegistry } from "./support/builtinObjectFactoryRegistry";

const objectFactory = createBuiltinObjectFactoryRegistry();
const pos = { x: 100, y: 200 };

describe("createObjectDoc", () => {
	it("the generated Doc has an id (UUID)", () => {
		const doc = createObjectDoc("rect", pos, objectFactory);
	...
	describe("rect", () => {
		...
		it("type is 'rect'", () => {
			expect(createObjectDoc("rect", pos, objectFactory).type).toBe("rect");
		});
	...
	describe("ellipse", () => {
		it("sets the position as cx/cy", () => {
			const doc = createObjectDoc("ellipse", pos, objectFactory);
		...
		it("type is 'ellipse'", () => {
			expect(createObjectDoc("ellipse", pos, objectFactory).type).toBe(
				"ellipse",
			);
		});
	});
	...
	describe("polyline", () => {
		it("has 2 horizontal points centered on the position", () => {
			const doc = createObjectDoc("polyline", pos, objectFactory);
		...
		it("type is 'polyline'", () => {
			expect(createObjectDoc("polyline", pos, objectFactory).type).toBe(
				"polyline",
			);
		});
	});
	...
	describe("polygon", () => {
		it("has 5 vertices", () => {
			const doc = createObjectDoc("polygon", pos, objectFactory);
			const pg = doc as unknown as { points: Array<{ x: number; y: number }> };
			expect(pg.points).toHaveLength(5);
		});

		it("type is 'polygon'", () => {
			expect(createObjectDoc("polygon", pos, objectFactory).type).toBe(
				"polygon",
			);
		});
