...
/** iTXt keyword under which the `.jis` source is stored in exported PNGs. */
export const PNG_SOURCE_KEYWORD = "jiscribe";

/**
 * Embeds a CanvasDoc (the `.jis` content) into a PNG as an `iTXt` chunk,
...
/**
 * Extracts the `.jis` text embedded by {@link embedCanvasSourceInPng}.
