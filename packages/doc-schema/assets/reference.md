...
| `view`       | `ViewDoc`     | -        | Display declaration: `padding` (empty space in world px outside the content — `top`/`right`/`bottom`/`left`, each optional, default 0) and `open` (`"fit-width"` \| `"fit-all"`, how the view is framed on open). Presentation only: objects may sit outside the padding, editing and scrolling are not constrained by it. Omitted = the host frames the document however it likes. |
