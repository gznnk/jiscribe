...
![Jiscribe で描いた 3 層構成の AWS アーキテクチャ図](./docs/images/hero-aws-architecture.jis.png)
...
|                                                                                                                                                                                        |                                                                                                                                                                                           |
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ![アイソメトリック格子の上に描いたクラウド構成図](./docs/images/gallery-isometric-topology.jis.png)<br>**正確な寸法のインフラ図** — 立体はすべて 30 度格子に沿ったネイティブなポリゴン | ![請求書コンソールのモック](./docs/images/gallery-billing-console.jis.png)<br>**UI モック** — 表・ツールバー・状態表示を、仕様書に使える精度で                                            |
| ![注文ドメインの UML クラス図](./docs/images/gallery-uml-class-diagram.jis.png)<br>**UML・クラス図** — パッケージ、インターフェース、コンポジション、多重度、ノート                    | ![1 枚に収めた障害のポストモーテム](./docs/images/gallery-postmortem.jis.png)<br>**1 枚のキャンバスに文書を丸ごと** — ポストモーテム: 数字・タイムライン・原因・アクション                |
...
上の画像はどれも `.jis.png` である。書き出した画像に描画元の文書が埋め込まれているので、
