...
Lucide アイコン・AWS アーキテクチャといった図形セットはプラグインとして同梱される。
図形セットは今後も増やしていく。
...
このギャラリーの画像はすべて `.jis.png` で、書き出した絵の中に、それを描いた文書が
入っている。画像を貼る場所ではただの画像として使え、Jiscribe で開けば元の文書として
編集できる。どれも特別なモードで描いたものではない。図形とテキストとコネクタだけの
ふつうの文書で、AI が書き、人が手で動かせるものである。
...
必要なのは Node 22 以降だけである。npm パッケージ
[`jiscribe-mcp`](https://www.npmjs.com/package/jiscribe-mcp) にサーバー・ビューア・
テキスト計測用のフォントがすべて含まれている。ツールの一覧とビューアの挙動は
[`apps/mcp`](./apps/mcp) を参照。

Claude Code なら、プラグインがこの登録を済ませたうえで、図形セットから生成した
作図スキルを追加する:
...
measure・capture で自分の描いたものを確かめながら描き、人はその過程を VSCode の
キャンバスで見ていられる。

何も入れずに手で描いてみたいだけなら、[Jiscribe Web](https://app.jiscribe.dev/)
がブラウザ上でキャンバスを開ける。アカウント登録もアップロードも無く、ファイルは
手元のマシンに置いたままである。

## エンジンを組み込む（準備中）

パッケージ本体はまだ npm に公開していない。`@jiscribe/canvas` などは組み込み向けの
API を整備している段階で、公開 API は今後変わる可能性がある。
