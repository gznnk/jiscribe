...
`@jiscribe/canvas` の作例集。サイドバーは 2 セクションに分かれる。

- **Gallery** — 完成した作例をそのまま描画する。何が描けるものなのかを最初に見せるための
  セクションで、`diagrams/` から数点だけを選んで載せる
- **Examples** — 統合例。1 example = 1 ファイル（`src/examples/*.tsx`）で、各ファイルは
  単体でコピペ可能な統合例として読めるように書く（シェルに依存しない。親要素いっぱいに
  描画する、が唯一の契約）

```bash
pnpm dev:examples     # 起動（http://localhost:5174/）
...
絶対値なので、作例を足すときは実際に開いて数値を合わせる。
作例 .jis.json に横断的な正本は無く、使う側がそれぞれ自分の分を持つ（ランディングページの
デモギャラリーは自分の `public/demo/diagrams/` を持っている）。
...
| `file-io`       | .jis の読み込み・保存、エクスポート PNG のドロップ復元                                      |
...
2. `src/ExamplesShell.tsx` の `EXAMPLE_ENTRIES` に登録する
