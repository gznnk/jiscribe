...
import { calcNoteTextRegion } from "../textRegions";

describe("calcNoteTextRegion", () => {
	it("gives up the fold on the right and keeps the whole height", () => {
		const region = calcNoteTextRegion({ width: 180, height: 110 });
	...
	it("stops short of the fold's inner corner, so no line can run under it", () => {
		...
		const region = calcNoteTextRegion({ width, height });
	...
	it("follows the fold when the shorter side changes which one it is taken from", () => {
		...
		const portrait = calcNoteTextRegion({ width: 110, height: 300 });
		const landscape = calcNoteTextRegion({ width: 300, height: 110 });
	...
	it("returns the box itself for a zero-width note instead of NaN", () => {
		const region = calcNoteTextRegion({ width: 0, height: 110 });
