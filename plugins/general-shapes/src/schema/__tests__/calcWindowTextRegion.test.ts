...
import { calcWindowTextRegion } from "../textRegions";

describe("calcWindowTextRegion", () => {
	it("starts the text below the title bar, so a line cannot run into it", () => {
		const region = calcWindowTextRegion({ width: 200, height: 100 });
	...
	it("scales with the box, since the insets are ratios rather than pixels", () => {
		const small = calcWindowTextRegion({ width: 200, height: 100 });
		const large = calcWindowTextRegion({ width: 400, height: 200 });
