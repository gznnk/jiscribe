import type { CreateObjectType, ObjectFeatures } from "@workspace/canvas/doc";
import {
	AUTO_COLOR,
	DEFAULT_FONT_FAMILY,
} from "@workspace/canvas/unstable-doc";

/**
 * A file with a folded top-right corner, used for source files and configuration.
 *
 * It adopts rect geometry (x/y/width/height) and only swaps the rendering. This
 * lets it reuse Frame-based transforms and connector outline connections with
 * the same mechanism as Rect. The text is laid out inside the drawing, clear of its details.
 */
export const FileFeatures = {
	type: "file",
	geometry: "rect",
	transform: true,
	stroke: true,
	fill: true,
	text: "body",
	connectable: true,
} as const satisfies ObjectFeatures;

/**
 * The folded corner, as fractions of the width and of the height. The smaller of
 * the two wins (calcFileFoldSize), so the fold stays a right isosceles triangle
 * at any aspect ratio. Shared by the silhouette, the outline and the text region.
 */
export const FILE_FOLD_WIDTH_RATIO = 0.3;
export const FILE_FOLD_HEIGHT_RATIO = 0.28;

// eslint-disable-next-line @typescript-eslint/no-unused-vars
declare const FileDocBrand: unique symbol;

export type FileDoc = CreateObjectType<
	typeof FileFeatures,
	typeof FileDocBrand
>;

export const FILE_DOC_DEFAULTS: Omit<FileDoc, "id"> = {
	type: "file",
	x: 0,
	y: 0,
	width: 100,
	height: 120,
	fill: "transparent",
	stroke: AUTO_COLOR,
	strokeWidth: 2,
	text: "",
	textAlign: "center",
	verticalAlign: "middle",
	fontColor: AUTO_COLOR,
	fontSize: 16,
	fontFamily: DEFAULT_FONT_FAMILY,
	fontWeight: "normal",
} as const as FileDoc;
