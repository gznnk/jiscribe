...
const regionOf = (
	...
	height: number,
	slot?: Partial<TextSlot>,
): ReturnType<typeof calcActorTextRegion> =>
	calcActorTextRegion(actor(width, height, slot), BODY_TEXT_SLOT_ID);

/** The label box of a single line at the default font size. */
const singleLineHeight =
	ACTOR_LABEL_STYLE_DEFAULTS.fontSize * TEXT_LINE_HEIGHT + 2 * 2;
...
describe("calcActorTextRegion", () => {
	it("hangs the label below the box, horizontally centered on the origin", () => {
		...
		expect(region.y).toBeCloseTo(50 + ACTOR_LABEL_GAP);
	...
	it("reserves one more line per authored newline", () => {
		...
		expect(twoLines.height).toBeCloseTo(
			oneLine.height + ACTOR_LABEL_STYLE_DEFAULTS.fontSize * TEXT_LINE_HEIGHT,
		);
	});
	...
	it("stays a valid box for a zero-sized shape rather than collapsing", () => {
		...
		expect(region.y).toBeCloseTo(ACTOR_LABEL_GAP);
