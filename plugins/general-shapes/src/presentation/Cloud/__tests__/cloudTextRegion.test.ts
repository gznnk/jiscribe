...
describe("cloud textRegion", () => {
	it("insets the box by the bump ratios and stays centered on the origin", () => {
		const region = calcCloudTextRegion(
	...
	it("keeps the insets proportional to the box size", () => {
		const small = calcCloudTextRegion(
			{ width: 100, height: 50 },
			"body",
			TEXT_REGION_CONTEXT,
		);
		const large = calcCloudTextRegion(
			{ width: 200, height: 100 },
			"body",
			TEXT_REGION_CONTEXT,
		);
	...
	it("collapses to zero for a zero-sized box", () => {
		expectRectCloseTo(
			calcCloudTextRegion({ width: 0, height: 0 }, "body", TEXT_REGION_CONTEXT),
			{
				x: 0,
				y: 0,
				width: 0,
